...
const Navigation = () => {
    ...
    return ( <
        ...
        { /* LOGO */ } <
        img className = "h-7"
        src = { `${apiRequest}/uploads/system/Logo.svg` }
        alt = "logo" / >

        { /* DESKTOP MENU */ } <
        ul className = "hidden items-center gap-6 lg:flex lg:font-bold lg:text-gray-400" >
        <
        li > < Link className = 'p-2 hover:text-[#359740] transition'
        to = "/#catalog" > ALL PRODUCTS < /Link></li >
        <
        li > < Link className = 'p-2 hover:text-[#359740] transition'
        to = "/aboutSeedra" > ABOUT SEEDRA < /Link></li >
        <
        li > < Link className = 'p-2 hover:text-[#359740] transition'
        to = "/ourBlog" > OUR BLOG < /Link></li >

        {
            checkToken ? ( <
                li >
                <
                button className = 'p-2 hover:text-[#359740] transition'
                onClick = {
                    () => openReport("OTHER", checkToken.id) } > SUPPORT < /button> <
                /li>
            ) : ( <
                li >
                <
        ...
        <
        input className = "h-8 w-52 rounded-[10px] border-gray-300 p-3 border px-2 text-sm"
        placeholder = "Search"
        onInput = {
            () => setStateSearch(true) }
        onChange = {
...

        {
            stateSearch && ( <
                ...
                <
                button className = "p-3 text-sm hover:bg-gray-200"
                onClick = {
                    () => {
        ...
        {!checkToken ? ( <
                >
                <
                Link className = 'p-2 hover:text-[#359740] transition'
                ...
                Link className = 'p-2 hover:text-[#359740] transition'
                to = { `/profile/${checkToken.id}` } > Profile < /Link>
            )
        }
        ...
        <
        input className = "h-8 w-40 rounded-[10px] border-gray-300 p-3 border px-2 text-sm"
        placeholder = "Search"
        onInput = {
            () => setStateSearch(true) }
        onChange = {
        ...
        button className = "text-[20px] lg:hidden"
        onClick = {
        ...
        { /* MOBILE MENU */ } {
            menuOpen && ( <
                ...
                <
                Link className = 'p-3 text-center bg-gray-300'
                to = "/#catalog" > All products < /Link> <
                Link className = 'p-3 text-center bg-gray-300'
                to = "/aboutSeedra" > About < /Link> <
                Link className = 'p-3 text-center bg-gray-300'
                to = "/ourBlog" > Blog < /Link>
                ...
                <
                button className = 'p-3 text-center bg-gray-300'
                onClick = {
                    () => {
                        reqBasket()
                        openBasketModal()
                ...
                onClick = {
                    ...
                    checkToken ? ( <
                        >
                        <
                        button className = 'p-3 text-center bg-red-300 hover:text-[#359740] transition'
                        onClick = {
                            () => openReport("OTHER", checkToken.id) } > Support < /button> <
                        />
                    ) : ( <
                        >
                        <
                        button className = 'p-2 bg-red-300 hover:text-[#359740] transition'
                        onClick = {
                            () => {
                                setNameResult("Помилка")
                                setDescResult("Треба бути зареєстрованим")
