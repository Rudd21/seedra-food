    ...
    return ( <
        ...
        div className = 'flex justify-around flex-col lg:flex-row' > {
            checkToken ? .id === userId ? ( <
                ...
                <
                img className = 'w-50 h-50 m-3 rounded-xl  bg-white'
                src = { `${apiRequest}/uploads/users/${checkToken?.avatar}` }
                alt = "" / > < /div> <
                ...
                /> <
                /label> <
                button className = 'bg-green-400 p-3 my-3 mx-10 hover:bg-green-600 transition'
                onClick = { changeAvatar } > Змінити аватарку < /button> <
                /div> <
                ...
                button className = 'w-35 h-15 bg-red-400 p-4 m-3 hover:bg-red-700 hover:text-white transition'
                onClick = {
                    () => openReport("USER", userInfo.id) } > Поскаржитись < /button> <
                /div>
            )
        } {
            checkToken ? .id && ( <
                ...
                <
                button className = 'p-4 rounded-md hover:bg-gray-200 transition'
                onClick = {
                    () => {
                ...
                Link className = 'p-4 rounded-md text-center hover:bg-gray-200 transition'
        ...
        div className = 'grid mx-auto my-[5px] grid-cols-1 sm:grid-cols-5 gap-5' > {
            userCatalog && userCatalog.length > 0 ? (
                userCatalog.map((userCata) => ( <
                    div key = { userCata.id } >
                    <
                    div className = "border p-3 h-[100%] border-gray-300 rounded-xl"
                    data - hashtag = { userCata.type }
                    onClick = {
                        () => { reqCommentsProduct(userCata.id) } } >
                    <
                    div className = "safe-productaImage" > {
                        checkToken ? .id === userId ? ( <
                            div className = 'flex justify-between' >
                            <
                            button className = 'bg-yellow-400 hover:bg-yellow-700 transition'
                            onClick = {
                                () => openEditProductModal(userCata.id) } >
                            <
                            img className = 'w-7 h-7 p-1'
                            src = "/edit.png"
                            alt = "" / >
                            <
                            /button> <
                            button className = 'bg-red-400 hover:bg-red-700 transition'
                            onClick = {
                                () => deleteProduct(userCata.id) } >
                            <
                            img className = 'w-7 h-7 p-1'
                            src = "/delete.png"
                            alt = "" / >
                    ...
                    img src = { `${apiRequest}/uploads/products/${userCata.image}` }
                    alt = "Product" / >
                    <
                    /div> <
                    p className = "flex" >
                    ...
                    key = { userCata.id } > { userCata.name } < /h3> <
                    ...
                    button className = 'h-10 p-2 text-green-700 border border-[#CBCBCB] rounded-md hover:bg-gray-300 transition duration-300'
                    onClick = {
                        () => toProduct(product.id) } > Discover < /button> <
                    /div> <
                    /div> <
                    /div>
                ))) : ( <
        ...
        div > { /* <p><strong>Comment ID:</strong> {comment.id}</p> */ } {
            productComments && productComments.length > 0 ? (
                ...
                .map((comment) => ( <
                    ...
                    src = { `${apiRequest}/uploads/users/${comment.user.avatar}` }
                    alt = "" / >
                    <
                    ...
                    <
                    button className = 'w-25 h-10 bg-gray-300 p-2 m-1 hover:bg-gray-600 hover:text-white transition'
                    onClick = {
                        () => { toReply(comment.id) } } > Reply < /button>   <
                    /div> <
                    button className = 'w-10 h-10 bg-red-400 self-center m-3 text-[20px] text-white hover:bg-red-700 hover:text-white transition'
                    onClick = {
                        () => {
        ...
        div className = 'flex' >
        <
        button className = 'bg-green-200 m-1 p-1 rounded-xs hover:bg-green-400 transition'
        onClick = {
            () => setFilterOrder('PENDING') } > PENDING < /button> <
        button className = 'bg-green-400 m-1 p-1 rounded-xs hover:bg-yellow-400 transition'
        onClick = {
            () => setFilterOrder('PAID') } > PAID < /button> <
        button className = 'bg-yellow-300 m-1 p-1 rounded-xs hover:bg-green-700 transition'
        onClick = {
            () => setFilterOrder('PROCESSING') } > PROCESSING < /button> <
        button className = 'bg-purple-400 m-1 p-1 rounded-xs hover:bg-red-400 transition'
        onClick = {
            () => setFilterOrder('SHIPPED') } > SHIPPED < /button> <
        button className = 'bg-green-500 m-1 p-1 rounded-xs hover:bg-red-400 transition'
        onClick = {
            () => setFilterOrder('COMPLETED') } > COMPLETED < /button> <
        button className = 'bg-red-200 m-1 p-1 rounded-xs hover:bg-red-400 transition'
        onClick = {
            () => setFilterOrder('CANCELLED') } > CANCELLED < /button> <
        /div>
        ...
        div className = 'max-h-[30vh] min-h-[30vh] overflow-y-scroll' > {
            ownerOrders && ownerOrders.length > 0 ? (
                ...
                .map(order => ( <
                    ...
                    label
                    for = 'statues' > Статус замовлення: < /label> <
                    select className = 'bg-gray-300 m-2 p-1 text-l'
                    onChange = {
                        (e) => handleOrderStatus(order.publicToken, e.target.value) }
                    name = "status"
                    id = "statues"
