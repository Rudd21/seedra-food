    ...
    return ( <
        div className = "fixed inset-0 bg-black/50 z-50 flex items-center justify-center" > {
            windowType === 'username' ? ( <
                ...
                <
                button className = 'text-red-500 ml-70'
                onClick = { closeEditProfileModal } > x < /button> <
                p className = 'm-3' > Введіть новий username: < /p> <
                input className = 'm-3 bg-white border rounded-xs'
                type = "text"
                onChange = {
                    (e) => setUsername(e.target.value) }
                /> <
                button className = 'p-3 bg-green-500 hover:bg-green-700 transition'
                onClick = { changeUsername } > Змінити < /button> <
                /div>
            ) : ( <
                div className = 'bg-white p-4 rounded shadow-lg' >
                <
                ...
                type = "password"
                onChange = {
                    (e) => setRepeatNewPassword(e.target.value) }
                /> <
                button disabled = { repeatNewPassword !== newPassword }
                className = "
                p - 3 rounded bg - green - 500 hover: bg - green - 700 disabled: bg - gray - 400 disabled: cursor - not - allowed transition "
