...
export const BasketModal = () => {
    ...
    return ( <
        ...
        button className = 'm-5 bg-green-700 p-1 text-white'
        onClick = {
            () => closeBasketModal() } > Close < /button> <
        /div> {
            basketList.length === 0 ? ( <
                ...
                basketList.map((product) => ( <
                    div key = { product.id }
                    className = 'flex flex-col w-[80%] lg:w-[100%] items-end m-3 p-3 border' >
                    <
                    button className = 'w-6 h-6 color-red bg-red-700 text-white'
                    onClick = {
                    ...
                    src = { `${apiRequest}/uploads/products/${product.image}` }
                    alt = "" / >
                    <
                    ...
                    Link className = 'p-1 w-[50%] text-center bg-blue-400 text-white hover:bg-blue-600 transition'
                    to = { `/productPage/${product.id}` } > Product page < /Link> <
                    /div> <
                    /div> <
                    /div> <
                    /div>
